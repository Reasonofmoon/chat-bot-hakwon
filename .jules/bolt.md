...
## 2025-02-24 - SVG Optimization
**Learning:** Replacing `opacity` with `strokeOpacity` on SVG elements with only strokes avoids potential compositing layer creation.
**Action:** Use specific opacity attributes (`strokeOpacity`, `fillOpacity`) instead of generic `opacity` when animating or rendering simple SVG shapes.
