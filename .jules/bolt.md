...
**Learning:** For highly reused components like Buttons, creating new style objects on mount (even inside `useMemo`) adds unnecessary allocation overhead. Pre-defining static style objects for common states (e.g., loading, disabled) and returning them directly avoids allocation entirely for default cases.
...
## 2025-02-23 - SVG Animation Performance
**Learning:** Continuous animations (CSS or SMIL) can cause main-thread work. Adding `will-change: transform` hints the browser to promote the element to a compositor layer, smoother animations.
**Action:** Always add `will-change: transform` to elements with continuous rotation or transform animations.

## 2025-02-23 - Inline SVG Bundle Size
**Learning:** React 19 and modern browsers do not require the `xmlns` attribute for inline SVGs. Removing it reduces bundle size without affecting functionality.
**Action:** Remove `xmlns` from all inline SVG components intended for HTML5 documents.
