...
## 2026-02-03 - SVG Optimization
**Learning:** When animating SVG strokes where `fill="none"`, using `strokeOpacity` instead of `opacity` prevents the browser from creating unnecessary compositing layers.
**Action:** Applied `strokeOpacity` and extracted static children in `Spinner` to reduce allocation and rendering overhead.
